...
class Signaling {
  ...
  Signaling(this._host, this._displayName, this._selfId, this.context);
  ...
  void invite(String peer_id, String media, use_screen, {isHome = false}) {
    this.sessionId = this._selfId + '-' + peer_id;

    if (this.onStateChange != null) {
      this.onStateChange(SignalingState.CallStateNew);
    }

    var controller = Provider.of<CallController>(context);

    for (var item in controller.onlineUsers.data) {
      if (item.id == peer_id) {
        print(' Youre calling: ' + item.name);
        controller.callerName = item.name;
      }
    }
    if (!isHome) {
    ...
    _createPeerConnection(peer_id, media, use_screen, isHome).then((pc) {
      ...
      case 'peers':
        {
          ...
          controller.onlineUsers = WebRTCUsers.fromJson(message);
        }
        break;
      case 'offer':
        {
          ...
          if (controller.onlineUsers != null)
            for (var item in controller.onlineUsers.data) {
              if (item.id == id) {
                print(item.name + ' is calling you');
                controller.callerName = item.name;
              }
            }
          ...
          var pc = await _createPeerConnection(id, media, false, true);
          ...
          controller.streamController.stream.asBroadcastStream().listen(
              (data) async {
            print("DataReceived from Stream: " + data);
            if (data == "answer") {
              //when user answers
              await _createAnswer(id, pc, media).then((_) {
    ...
    try {
      ...
      */
      ...
      _socket.listen((data) {
        ...
        this.onMessage(decoder.convert(data), context);
      }, onDone: () {
  ...
  _createPeerConnection(id, media, user_screen, isHome) async {
    if (media != 'data' /* && media != 'audio' */)
